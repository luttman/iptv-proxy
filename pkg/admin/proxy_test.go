...
func TestProxySettingsSaveAndAccess(t *testing.T) {
	...
	resp, err = client.PostForm(ts.URL+"/admin/settings/proxy/test", url.Values{})
	if err != nil {
		t.Fatal(err)
	}
	resp.Body.Close() // nolint: errcheck
	...
	resp, err = client.PostForm(ts.URL+"/admin/settings/proxy/test", url.Values{"csrf_token": {token}, "proxy_url": {"http://host:bad"}})
	if err != nil {
		t.Fatal(err)
	}
	resp.Body.Close() // nolint: errcheck
	...
	form := url.Values{"proxy_url": {"http://name:secret@proxy.example.com:3128"}, "enabled": {"on"}}
	...
	if !strings.Contains(string(body), `type="text" value="http://name:secret@proxy.example.com:3128"`) {
